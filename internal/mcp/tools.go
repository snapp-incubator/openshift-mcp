...
func buildTools() []tool {
	return []tool{
		...
		{
			name:        "list_resource",
			description: "List any resource type (including CRDs and OpenShift kinds) by group/version/resource plural — servicemonitors, podmonitors, prometheusrules, cronjobs, hpa, networkpolicies, httpproxies, argo applications, database clusters and so on. Returns name/namespace/age summaries; filter server-side with namespace, selector or field_selector.",
			schema: objSchema(map[string]any{
				...
				"field_selector": str("Optional field selector, e.g. 'status.phase=Running' — filtered server-side."),
				"limit":          integer("Maximum items to return (default 100, max 200)."),
			}, "version", "resource"),
